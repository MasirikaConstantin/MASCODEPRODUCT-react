...
export default function CommentAstuces({ comment, auth, astuceId }) {
    // Utilisez le astuceId passé en prop OU celui du commentaire
    const actualPostId = astuceId || comment.astuce_id;
    const [showReplies, setShowReplies] = useState(false);
    const [showReplyForm, setShowReplyForm] = useState(false);
    const { data, setData, post, processing, reset } = useForm({
        contenus: '',
        parent_id: comment.id??"",
        astuce_id: astuceId || comment.astuce_id,
    });
    const handleReplySubmit = (e) => {
        e.preventDefault();
        post(route('commentsastuces.store', { astuce: actualPostId }), {
            preserveScroll: true,
            onSuccess: (response) => {
                reset('contenus');
            ...
            <div className="flex gap-3">
                ...
                <div className="flex-1">
                    ...
                    <LikeButton         
                                            ...
                                            likeableType="App\Models\CommentaireAstuce"
