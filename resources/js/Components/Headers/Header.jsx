...
const Header = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Seulement si la différence est significative pour éviter les bascules fréquentes
    ...
    <>
      ...
      {showTopBar && (
        <div className="bg-base-300 text-base-content border-b border-base-200/50 text-sm">
          ...
          <span>✨ Nouveaux articles chaque semaine</span>
          <button 
      ...
      <header 
        ref={headerRef}
        className={`navbar  sticky top-0 z-50 px-0  sm:px-4 transition-all duration-300 ease-out  ${
      ...
      >
        <div className="navbar-start w-full sm:w-auto mr-36  ">
          {/* Menu Mobile (Drawer) */}
          ...
          <Link
            href={route('home')} 
            className="logo btn btn-ghost normal-case text-xl px-2 mx-auto sm:mx-0 relative overflow-hidden me-16"
            style={{
              '--x': '0px',
              '--y': '0px',
              '--glow-opacity': '0'
          ...
          >
            <span className="text-primary transition-all duration-300 hover:text-primary-focus font-bold me-6">Mascode</span>
            <span className="transition-all duration-300 hover:text-base-content/80">Blog</span>
            <span 
